...
void calculateControlTorques(const RobotState *state, double *tau)
{
    for (int i = 0; i < 3; i++)
    {
        // PD Control Law: tau = Kp * (desired_pos - current_pos) + Kd * (desired_vel - current_vel)
        tau[i] = Kp[i] * (th0dds[i] - state->q[i]) + Kd[i] * (th1dds[i] - state->qdot[i]);

        // Print the control torque equation for each joint
        printf("%f*th0dds%d + %f*th1dds%d + th2dds%d - %f*x%d - %f*x%d\n",
               Kp[i], i + 1, Kd[i], i + 1, i + 1, Kp[i], i + 1, Kd[i], i + 4);
    }
}
